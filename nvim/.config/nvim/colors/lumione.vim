...
"    let s:hue_6_2 = 'c18401' " orange 2
    ...
    let s:hue_1  = '000000' " black 1
    let s:hue_2  = 'B8520E' " brown 1
    let s:hue_3  = '265AA1' " blue 1
    let s:hue_4  = '08961E' " green 1

    let s:hue_5   = '563a89' " violet 1
    let s:hue_5_2 = '563a89' " violet 1

    let s:hue_6   = '563a89' " violet 1
    let s:hue_6_2 = '563a89' " violet 1


" custom start

    let s:hue_7   = '563a89' " violet 1

    let s:hue_8   = 'B8520E' " brown 1

    let s:hue_9   = '08961E' " green 1

    let s:hue_10   = '265AA1' " blue 1

    let s:hue_11   = '8A8A8A' " gray 1
...
" custom end
  ...
  call <sid>X('Todo',     s:hue_5,         s:syntax_bg,      'none')
  ...
  call <sid>X('cssAttributeSelector', s:hue_5,  '', '')
  call <sid>X('cssBraces',            s:mono_2, '', '')
  call <sid>X('cssClassName',         s:hue_6,  '', '')
  call <sid>X('cssClassNameDot',      s:hue_6,  '', '')
  call <sid>X('cssDefinition',        s:hue_1,  '', '')
  call <sid>X('cssFontAttr',          s:hue_4,  '', '')
  call <sid>X('cssFontDescriptor',    s:hue_3,  '', '')
  call <sid>X('cssFunctionName',      s:hue_5,  '', '')
  call <sid>X('cssIdentifier',        s:hue_2,  '', '')
  call <sid>X('cssImportant',         s:hue_3,  '', '')
  call <sid>X('cssInclude',           s:hue_3,  '', '')
  call <sid>X('cssMediaKeyword',      s:hue_1,  '', '')
  ...
  call <sid>X('cssSelectorOp',        s:hue_1,  '', '')
  call <sid>X('cssSelectorOp2',       s:hue_1,  '', '')
  call <sid>X('cssStringQ',           s:hue_4,  '', '')
  call <sid>X('cssStringQQ',          s:hue_4,  '', '')
  call <sid>X('cssTagName',           s:hue_5,  '', '')
  call <sid>X('cssAttr',              s:hue_4,  '', '')
  call <sid>X('cssValueLength',       s:hue_4,  '', '')
  call <sid>X('cssValueTime',         s:hue_4,  '', '')
  call <sid>X('cssValueNumber',         s:hue_4,  '', '')
  call <sid>X('cssUnitDecorators',    s:hue_4,  '', '')


  call <sid>X('sassDefinition',     s:hue_1,   '', '')
  ...
  call <sid>X('scssAttribute',      s:hue_4,   '', '')
  call <sid>X('scssAmpersand',      s:hue_1,   '', '')
  call <sid>X('scssVariable',       s:hue_3,   '', '')
  call <sid>X('scssSelectorName',   s:hue_6_2, '', '')
  call <sid>X('scssSelectorChar',   s:hue_5, '', '')
  " }}}
  ...
  " HTML highlighting -------------------------------------------------------{{{
  call <sid>X('htmlArg',            s:hue_1,  '', '')
  call <sid>X('htmlString',         s:hue_9,  '', '')
  call <sid>X('htmlTagName',        s:hue_5,  '', '')
  call <sid>X('htmlTagN',           s:hue_5,  '', '')
  call <sid>X('htmlSpecialTagName', s:hue_5,  '', '')
  call <sid>X('htmlTag',            s:hue_6, '', '')
  call <sid>X('htmlEndTag',         s:mono_2, '', '')
  call <sid>X('htmlLink',         s:hue_1, '', '')
  call <sid>X('htmlH1',         s:hue_1, '', '')

  call <sid>X('MatchTag',   s:hue_5,         s:syntax_cursor,  'underline,bold')
  " }}}

  " Twig highlighting -------------------------------------------------------{{{
    call <sid>X('twigVariable',            s:hue_3,  '', '')
    call <sid>X('twigStatement',            s:hue_5,  '', '')
    call <sid>X('twigTagDelim',            s:hue_1,  '', '')
    call <sid>X('twigVarDelim',            s:hue_1,  '', '')
    " }}}
  ...
  call <sid>X('jsBraces',               s:hue_1,  '', '')
  call <sid>X('jsBracket',               s:hue_3,  '', '')
  call <sid>X('jsBrackets',               s:hue_1,  '', '')
  call <sid>X('jsClassBraces',          s:hue_1,  '', '')
  call <sid>X('jsClassKeywords',        s:hue_3,   '', '')
  call <sid>X('jsDocParam',             s:hue_2,   '', '')
  call <sid>X('jsDocTags',              s:hue_3,   '', '')
  call <sid>X('jsFuncBraces',           s:hue_1,  '', '')
  ...
  " basic
  call <sid>X('phpRegion',    s:hue_12, '', '')
  call <sid>X('phpOperator',    s:hue_12, '', '')
  call <sid>X('phpParent',  s:hue_12,   '', '')
  call <sid>X('phpStructure',    s:hue_7, '', '')
  call <sid>X('phpInclude',      s:hue_7,   '', '')
  call <sid>X('phpStorageClass',      s:hue_7,   '', '')
  call <sid>X('phpIdentifier',      s:hue_10,   '', '')
  call <sid>X('phpVarSelector',  s:hue_10,   '', '')

  " namespace
  call <sid>X('phpNamespace',        s:hue_8, '', '')

  " classes
  call <sid>X('phpUseClass',        s:hue_8, '', '')
  call <sid>X('phpKeyword',        s:hue_7, '', '')
  call <sid>X('phpClass',        s:hue_8, '', '')
  call <sid>X('phpClassNew',        s:hue_8, '', '')
  " method type
   call <sid>X('phpType',         s:hue_7,   '', '')
   call <sid>X('phpBoolean',      s:hue_2,   '', 'bold')
  " static class name
  call <sid>X('phpStaticClasses',        s:hue_8, '', '')
  call <sid>X('phpMemberSelector',        s:hue_12, '', '')
  call <sid>X('phpMethod',        s:hue_8, '', '')
  call <sid>X('phpMethodsVar',        s:hue_10, '', '')
  ...
  " functions
  call <sid>X('phpDefine',            s:hue_7,   '', '')
  call <sid>X('phpFunctions',    s:hue_8,   '', '')
  call <sid>X('phpFunction',     s:hue_8,   '', '')
  call <sid>X('phpFunctionCall',     s:hue_8,   '', '')


  " strings
  call <sid>X('phpStringSingle',      s:hue_9,   '', '')
  call <sid>X('phpStringDouble',      s:hue_9,   '', '')
...
"}}}
  " Public API --------------------------------------------------------------{{{
  function! lumione#highlight(group, fg, bg, attr)
